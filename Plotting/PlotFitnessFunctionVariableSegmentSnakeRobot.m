function [Score] = PlotFitnessFunctionVariableSegmentSnakeRobot(design,sample_size,Voxels,directory)
...
% this. This version plots each thing
...
clf
%Load Anatomy
PlotAnatomy(V) %The stl file
hold on
%Plot Voxels
disp('Making voxels')
%PlotVoxelspaceGoalsOnly(V) %The goals only
PlotVoxelspace(V)  
%Figure settings
grid on
axis equal
...
print(gcf,'FitnessFunctionTaskSpace.png','-dpng','-r600');
pause(0.001)
...
for ii = 1:N
    ...
    if (~(any(v==0)) && (V.Goal_labels(v(1),v(2),v(3)) == true))
        ...
        if (CheckCollision(V,Traj) == false)
          %Plot This configuration of Robot Reaching the Target:
          disp('Found a configuration')
          hold on
          ...
          disp('Picture Achieved End here...')
          %zoom(1.75)
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
print(gcf,'FitnessFunction.png','-dpng','-r600');
